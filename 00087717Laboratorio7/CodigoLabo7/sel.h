...
Matrix createLocalK(int e,mesh &m){
    ...
    Matrix matrixA,matrixF,matrixG,matrixD,matrixE;
    ...
    T = m.getParameter(Te);
    productRealMatrix(T*J/(6*D*4),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,Beta,2,2,6),6,2,6),matrixA);



    //Preparando matrixF (En clase conocida simplemente como F)
    Ka = m.getParameter(Ka);
    
    productRealMatrix(Ka*Ae/(D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,Beta,2,2,6),2,2,6),6,2,6),matrixF);


    //Preparando matrixG (En clase conocida simplemente como G)
    S = m.getParameter(Se);
    productRealMatrix((-1)*(S*Ae)/(8*D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixG);
    ...
    zeroes(K,9);
    ubicarSubMatriz(K,0,5,0,5,sumMatrix(matrixA,matrixF,6,6));
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixD,matrixG,6,3));
